...
import { Phone, Mail, MapPin, Calendar, CheckCircle2, Quote } from "lucide-react";
    ...
    <main>
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur border-b">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          <a href="#top" className="font-semibold tracking-tight">{t.brandName}</a>
          <nav className="hidden sm:flex gap-6 text-sm">
            <a href="#services" className="hover:text-brand-700">{t.nav.services}</a>
            <a href="#about" className="hover:text-brand-700">{t.nav.about}</a>
            <a href="#approach" className="hover:text-brand-700">{t.nav.approach}</a>
            <a href="#testimonials" className="hover:text-brand-700">{t.nav.testimonials}</a>
            <a href="#contact" className="hover:text-brand-700">{t.nav.contact}</a>
          </nav>
          <div className="flex items-center gap-3">
            <a href="#contact">
              <Button className="rounded-2xl hidden sm:inline-flex">{t.ctaPrimary}</Button>
            </a>
            <Button
              variant="outline"
          ...
          <motion.div {...fadeUp} className="grid md:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <div className="mt-6 flex gap-3">
                <a href="#contact"><Button className="rounded-2xl">{t.hero.getInTouch}</Button></a>
                <a href="#services"><Button variant="outline" className="rounded-2xl">{t.hero.explore}</Button></a>
              </div>
      ...
      <section id="services" className="mx-auto max-w-6xl px-4 py-16">
        <motion.h2 {...fadeUp} className="text-2xl md:text-3xl font-semibold">{t.services.title}</motion.h2>
      ...
      <section id="about" className="mx-auto max-w-6xl px-4 py-16">
        <div className="grid md:grid-cols-2 gap-10 items-start">
          ...
          <motion.div {...fadeUp} className="relative aspect-[5/4]">
            <Image src="/about-office.jpg" alt={t.about.officeAlt} fill className="object-cover rounded-2xl shadow-lg" />
          </motion.div>
        </div>
      </section>

      {/* Approach */}
      <section id="approach" className="mx-auto max-w-6xl px-4 py-16">
        <motion.h2 {...fadeUp} className="text-2xl md:text-3xl font-semibold">{t.approach.title}</motion.h2>
          ...
          {t.approach.steps.map((x) => (
            <Card key={x.step} className="rounded-2xl">
              <CardContent className="p-6">
                ...
                <div className="mt-2 text-sm text-stone-600">{x.text}</div>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <section id="testimonials" className="mx-auto max-w-6xl px-4 py-16">
        <motion.h2 {...fadeUp} className="text-2xl md:text-3xl font-semibold">{t.testimonials.title}</motion.h2>
        <div className="mt-8 grid md:grid-cols-2 gap-6">
          {t.testimonials.quotes.map((q, i) => (
            <blockquote key={i} className="rounded-2xl border p-6 bg-white">
              ...
              <p className="mt-3 text-stone-700">“{q}”</p>
            </blockquote>
          ))}
        </div>
      </section>
        ...
        <div className="grid md:grid-cols-2 gap-10">
          <div>
            ...
            <ul className="mt-6 space-y-3 text-stone-700">
              <li className="flex items-center gap-2"><Phone className="h-4 w-4" /><a href={`tel:${t.contact.phone}`}>{t.contact.phone}</a></li>
              <li className="flex items-center gap-2"><Mail className="h-4 w-4" /><a href={`mailto:${t.contact.email}`}>{t.contact.email}</a></li>
              <li className="flex items-center gap-2"><MapPin className="h-4 w-4" />{t.contact.location}</li>
              <li className="flex items-center gap-2"><Calendar className="h-4 w-4" />{t.contact.hours}</li>
            </ul>
          </div>
          <ContactForm t={t} />
        </div>
      ...
      <footer className="border-t bg-white">
        <div className="mx-auto max-w-6xl px-4 py-8 text-sm text-stone-600 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6">
            <a href="#top" className="hover:text-stone-900">{t.footer.backToTop}</a>
            <a href="/privacy" className="hover:text-stone-900">{t.footer.privacy}</a>
          </div>
        </div>
      </footer>
    </main>
...
function ContactForm({ t }: { t: typeof dict["en"] }) {
  return (
    <form
    ...
    >
      ...
      <Button type="submit" className="mt-5 rounded-2xl">{t.contact.form.send}</Button>
